...
def consultar_rae(palabra):
    ...
    def extraer_defs(soup_obj):
        """
        Estructura real del DLE (2024):
        - Definiciones en <li class="j"> o <li class="m">
          dentro de <ol class="c-definitions">
        - Sinónimos en <span class="sin">
        - Categoría gramatical en <abbr class="d" title="...">
        - Locuciones en <h3 class="k5">
        """
        defs = []
        sins = set()
        num_global = 0

        for li in soup_obj.find_all("li", class_=["j", "m"]):
            num_global += 1

            # Número de acepción
            num_tag = li.find("span", class_="n_acep")
            num     = num_tag.get_text(strip=True) if num_tag else str(num_global)

            # Categoría gramatical
            cat = " ".join(
                a.get("title", "") for a in li.find_all("abbr", class_="d")
                if a.get("title","") not in ("Sinónimos o afines",)
            ).strip()

            # Locución padre (h3.k5 anterior al li)
            locucion = ""
            for prev in li.find_all_previous("h3", class_="k5"):
                locucion = prev.get_text(strip=True)
                break

            # Texto: quitar el footer de sinónimos antes de extraer
            div_item = li.find("div", class_="c-definitions__item")
            if div_item:
                footer = div_item.find("div", class_="c-definitions__item-footer")
                if footer:
                    footer.decompose()
                texto = div_item.get_text(separator=" ", strip=True)
            else:
            ...
            if locucion:
                texto = f"[{locucion}] {texto}"

            if texto and len(texto) > 5:
                defs.append({"num": num, "categoria": cat, "texto": texto})

        # Sinónimos en <span class="sin">
        for span in soup_obj.find_all("span", class_="sin"):
...
if buscar and palabra.strip():
    ...
    with tab2:
        ...
        if rae["error"]:
            ...
            if "_html_crudo" in rae:
                st.download_button(
                    label     = "⬇️ Descargar HTML crudo de la RAE",
                    data      = rae["_html_crudo"].encode("utf-8"),
                    file_name = f"rae_{palabra}.html",
                    mime      = "text/html",
                    help      = "Descarga el HTML para analizar la estructura real"
                )
        else:
